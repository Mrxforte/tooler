...
class ToolAdapter extends TypeAdapter<Tool> {
  ...
  final int typeId = 0;
  @override
  Tool read(BinaryReader reader) => Tool.fromJson(reader.readMap().map((key, value) => MapEntry(key.toString(), value)));
  @override
...
class LocationHistoryAdapter extends TypeAdapter<LocationHistory> {
  ...
  final int typeId = 1;
  @override
  LocationHistory read(BinaryReader reader) => LocationHistory.fromJson(reader.readMap().map((key, value) => MapEntry(key.toString(), value)));
  @override
  void write(BinaryWriter writer, LocationHistory obj) => writer.writeMap(obj.toJson());
}
...
class ConstructionObjectAdapter extends TypeAdapter<ConstructionObject> {
  ...
  final int typeId = 2;
  @override
  ConstructionObject read(BinaryReader reader) => ConstructionObject.fromJson(reader.readMap().map((key, value) => MapEntry(key.toString(), value)));
  @override
  void write(BinaryWriter writer, ConstructionObject obj) => writer.writeMap(obj.toJson());
}
...
class NotificationAdapter extends TypeAdapter<AppNotification> {
  ...
  @override
  void write(BinaryWriter writer, AppNotification obj) => writer.writeMap(obj.toJson());
}
...
class Tool {
  ...
  factory Tool.fromJson(Map<String, dynamic> json) => Tool(
    ...
    locationHistory: (json['locationHistory'] as List?)?.map((e) => LocationHistory.fromJson(Map<String, dynamic>.from(e))).toList() ?? [],
  ...
  Tool copyWith({String? id, String? title, String? description, String? brand, String? uniqueId, String? imageUrl, String? localImagePath, String? currentLocation, String? currentLocationName, List<LocationHistory>? locationHistory, bool? isFavorite, DateTime? createdAt, DateTime? updatedAt, bool? isSelected, String? userId}) {
    return Tool(
      id: id ?? this.id,
  ...
  String? get displayImage => imageUrl?.isNotEmpty == true ? imageUrl : localImagePath?.isNotEmpty == true ? localImagePath : null;
}
...
class LocationHistory {
  ...
  String locationId;
  String locationName;
  LocationHistory({required this.date, required this.locationId, required this.locationName});
  factory LocationHistory.fromJson(Map<String, dynamic> json) => LocationHistory(
    date: json['date'] != null ? DateTime.parse(json['date'] as String) : DateTime.now(),
    locationId: json['locationId'] as String,
    locationName: json['locationName'] as String,
  );
  Map<String, dynamic> toJson() => {'date': date.toIso8601String(), 'locationId': locationId, 'locationName': locationName};
}
...
class ConstructionObject {
  ...
  ConstructionObject({
    ...
    this.isSelected = false,
    required this.userId,
  }) : toolIds = toolIds ?? [], createdAt = createdAt ?? DateTime.now(), updatedAt = updatedAt ?? DateTime.now();

  factory ConstructionObject.fromJson(Map<String, dynamic> json) => ConstructionObject(
    id: json['id'] as String,
  ...
  ConstructionObject copyWith({String? id, String? name, String? description, String? imageUrl, String? localImagePath, List<String>? toolIds, DateTime? createdAt, DateTime? updatedAt, bool? isSelected, String? userId}) {
...
// ========== ID GENERATOR ==========
class IdGenerator {
  static String generateToolId() => 'TOOL-${DateTime.now().millisecondsSinceEpoch}-${Random().nextInt(9999).toString().padLeft(4, '0')}';
  static String generateObjectId() => 'OBJ-${DateTime.now().millisecondsSinceEpoch}-${Random().nextInt(9999).toString().padLeft(4, '0')}';
...
class LocalDatabase {
  ...
  static Future<void> saveCacheTimestamp() async {
    try { await appSettings.put('last_cache_update', DateTime.now().toIso8601String()); } catch (e) {}
  }
  static Future<DateTime?> getLastCacheUpdate() async {
    try { final ts = appSettings.get('last_cache_update'); return ts != null ? DateTime.parse(ts) : null; } catch (e) { return null; }
  }
  static Future<bool> shouldRefreshCache({Duration maxAge = const Duration(hours: 1)}) async {
    try { final last = await getLastCacheUpdate(); return last == null || DateTime.now().difference(last) > maxAge; } catch (e) { return true; }
  }
}

class SyncItem {
  String id; String action; String collection; Map<String, dynamic> data; DateTime timestamp;
  SyncItem({required this.id, required this.action, required this.collection, required this.data, DateTime? timestamp}) : timestamp = timestamp ?? DateTime.now();
  Map<String, dynamic> toJson() => {'id': id, 'action': action, 'collection': collection, 'data': data, 'timestamp': timestamp.toIso8601String()};
}
...
class ImageService {
  ...
  static Future<File?> pickImage() async {
    try { final picked = await _picker.pickImage(source: ImageSource.gallery); return picked != null ? File(picked.path) : null; } catch (e) { return null; }
  }
  static Future<File?> takePhoto() async {
    try { final picked = await _picker.pickImage(source: ImageSource.camera); return picked != null ? File(picked.path) : null; } catch (e) { return null; }
  }
}
...
class NotificationProvider with ChangeNotifier {
  ...
  Future<void> loadNotifications(String userId) async {
    ...
    _notifications = LocalDatabase.notifications.values.where((n) => n.userId == userId).toList()
...
extension NotificationCopy on AppNotification {
  AppNotification copyWith({String? id, String? title, String? body, String? type, String? relatedId, String? userId, bool? read, DateTime? timestamp}) {
    return AppNotification(
      id: id ?? this.id,
      title: title ?? this.title,
      body: body ?? this.body,
...
class MoveRequestProvider with ChangeNotifier {
  ...
  List<MoveRequest> get pendingRequests => _requests.where((r) => r.status == 'pending').toList();
  List<MoveRequest> get userRequests => _requests.where((r) => r.requestedBy == FirebaseAuth.instance.currentUser?.uid).toList();

  Future<void> loadRequests() async {
    await LocalDatabase.init();
    _requests = LocalDatabase.moveRequests.values.toList()..sort((a, b) => b.timestamp.compareTo(a.timestamp));
    notifyListeners();
  }
...
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> signUpWithEmail(String email, String password, {File? profileImage, String? adminPhrase}) async {
    try {
      ...
      if (_user != null) {
        // Determine role based on admin phrase
...
class ToolsProvider with ChangeNotifier {
  ...
  List<String> get uniqueBrands {
    final brands = _tools.map((t) => t.brand).toSet().toList(); brands.sort(); return ['all', ...brands];
  }

  void toggleSelectionMode() { _selectionMode = !_selectionMode; if (!_selectionMode) _deselectAllTools(); notifyListeners(); }
  void toggleToolSelection(String toolId) { final index = _tools.indexWhere((t) => t.id == toolId); if (index != -1) { _tools[index] = _tools[index].copyWith(isSelected: !_tools[index].isSelected); notifyListeners(); } }
  void selectAllTools() { for (var i = 0; i < _tools.length; i++) _tools[i] = _tools[i].copyWith(isSelected: true); notifyListeners(); }
  void _deselectAllTools() { for (var i = 0; i < _tools.length; i++) _tools[i] = _tools[i].copyWith(isSelected: false); notifyListeners(); }

  void setFilterLocation(String location) { _filterLocation = location; notifyListeners(); }
  void setFilterBrand(String brand) { _filterBrand = brand; notifyListeners(); }
  void setFilterFavorites(bool value) { _filterFavorites = value; notifyListeners(); }
  void clearAllFilters() { _filterLocation = 'all'; _filterBrand = 'all'; _filterFavorites = false; _searchQuery = ''; notifyListeners(); }

  List<Tool> _getFilteredTools() {
    try {
      List<Tool> filtered = List.from(_tools);
      if (_filterLocation != 'all') filtered = filtered.where((tool) => tool.currentLocation == _filterLocation).toList();
      if (_filterBrand != 'all') filtered = filtered.where((tool) => tool.brand == _filterBrand).toList();
      if (_filterFavorites) filtered = filtered.where((tool) => tool.isFavorite).toList();
      if (_searchQuery.isNotEmpty) {
        final query = _searchQuery.toLowerCase();
        filtered = filtered.where((tool) => tool.title.toLowerCase().contains(query) || tool.brand.toLowerCase().contains(query) || tool.uniqueId.toLowerCase().contains(query) || tool.description.toLowerCase().contains(query)).toList();
      }
      return _sortTools(filtered);
    } catch (e) { return _sortTools(List.from(_tools)); }
  }
  ...
  List<Tool> _sortTools(List<Tool> tools) {
    tools.sort((a, b) {
      int cmp;
      switch (_sortBy) {
        case 'name': cmp = a.title.compareTo(b.title); break;
        case 'date': cmp = a.createdAt.compareTo(b.createdAt); break;
        case 'brand': cmp = a.brand.compareTo(b.brand); break;
  ...
  void setSort(String sortBy, bool ascending) { _sortBy = sortBy; _sortAscending = ascending; notifyListeners(); }
  ...
  Future<void> addTool(Tool tool, {File? imageFile}) async {
    ...
    try {
      ...
      if (imageFile != null) {
        ...
        if (url != null) tool = tool.copyWith(imageUrl: url);
        else tool = tool.copyWith(localImagePath: imageFile.path);
      }
      _tools.add(tool);
  ...
  Future<void> updateTool(Tool tool, {File? imageFile}) async {
    ...
    try {
      ...
      if (imageFile != null) {
        final userId = FirebaseAuth.instance.currentUser?.uid ?? 'local';
        final url = await ImageService.uploadImage(imageFile, userId);
        if (url != null) tool = tool.copyWith(imageUrl: url, localImagePath: null);
        else tool = tool.copyWith(localImagePath: imageFile.path, imageUrl: null);
      }
  ...
  Future<void> requestMoveTool(String toolId, String toLocationId, String toLocationName) async {
    ...
    final reqProvider = Provider.of<MoveRequestProvider>(navigatorKey.currentContext!, listen: false);
  ...
  Future<void> moveTool(String toolId, String newLocationId, String newLocationName) async {
    ...
    final updatedTool = tool.copyWith(
      locationHistory: [...tool.locationHistory, LocationHistory(date: DateTime.now(), locationId: oldLocationId, locationName: oldLocationName)],
  ...
  Future<void> _addToSyncQueue({required String action, required String collection, required Map<String, dynamic> data}) async {
    try { await LocalDatabase.syncQueue.put('${DateTime.now().millisecondsSinceEpoch}', SyncItem(id: '${DateTime.now().millisecondsSinceEpoch}', action: action, collection: collection, data: data)); } catch (e) { print('Sync queue error: $e'); }
  }
  ...
  Future<void> _syncWithFirebase() async {
    try {
      ...
      for (final item in LocalDatabase.syncQueue.values) {
        try {
          final doc = FirebaseFirestore.instance.collection('tools').doc(item.data['id'] as String);
          if (item.action == 'delete') await doc.delete();
          else await doc.set(item.data, SetOptions(merge: true));
          await LocalDatabase.syncQueue.delete(item.id);
        } catch (e) { print('Sync item error: $e'); }
      }
      final snapshot = await FirebaseFirestore.instance.collection('tools').where('userId', isEqualTo: user.uid).get();
...
class ObjectsProvider with ChangeNotifier {
  ...
  bool get isLoading => _isLoading;
  bool get selectionMode => _selectionMode;
  List<ConstructionObject> get selectedObjects => _objects.where((o) => o.isSelected).toList();
  bool get hasSelectedObjects => _objects.any((o) => o.isSelected);
  int get totalObjects => _objects.length;

  void toggleSelectionMode() { _selectionMode = !_selectionMode; if (!_selectionMode) _deselectAllObjects(); notifyListeners(); }
  void toggleObjectSelection(String objectId) { final index = _objects.indexWhere((o) => o.id == objectId); if (index != -1) { _objects[index] = _objects[index].copyWith(isSelected: !_objects[index].isSelected); notifyListeners(); } }
  void selectAllObjects() { for (var i = 0; i < _objects.length; i++) _objects[i] = _objects[i].copyWith(isSelected: true); notifyListeners(); }
  void _deselectAllObjects() { for (var i = 0; i < _objects.length; i++) _objects[i] = _objects[i].copyWith(isSelected: false); notifyListeners(); }

  List<ConstructionObject> _getFilteredObjects() {
    if (_searchQuery.isEmpty) return _sortObjects(List.from(_objects));
    final q = _searchQuery.toLowerCase();
    return _sortObjects(_objects.where((o) => o.name.toLowerCase().contains(q) || o.description.toLowerCase().contains(q)).toList());
  }
  ...
  List<ConstructionObject> _sortObjects(List<ConstructionObject> list) {
    list.sort((a, b) {
      ...
      switch (_sortBy) {
        ...
        case 'date': cmp = a.createdAt.compareTo(b.createdAt); break;
        case 'toolCount': cmp = a.toolIds.length.compareTo(b.toolIds.length); break;
  ...
  Future<void> loadObjects({bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (forceRefresh || await LocalDatabase.shouldRefreshCache()) await _syncWithFirebase();
    } catch (e) { print('Error loading objects: $e'); } finally { _isLoading = false; notifyListeners(); }
  }
  ...
  Future<void> addObject(ConstructionObject obj, {File? imageFile}) async {
    ...
    try {
      ...
      if (imageFile != null) {
        final userId = FirebaseAuth.instance.currentUser?.uid ?? 'local';
        final url = await ImageService.uploadImage(imageFile, userId);
        if (url != null) obj = obj.copyWith(imageUrl: url);
        else obj = obj.copyWith(localImagePath: imageFile.path);
      }
      _objects.add(obj);
  ...
  Future<void> updateObject(ConstructionObject obj, {File? imageFile}) async {
    ...
    try {
      ...
      if (imageFile != null) {
        final userId = FirebaseAuth.instance.currentUser?.uid ?? 'local';
        final url = await ImageService.uploadImage(imageFile, userId);
        if (url != null) obj = obj.copyWith(imageUrl: url, localImagePath: null);
  ...
  Future<void> _syncWithFirebase() async {
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) return;
      final snapshot = await FirebaseFirestore.instance.collection('objects').where('userId', isEqualTo: user.uid).get();
      for (final doc in snapshot.docs) {
        final obj = ConstructionObject.fromJson({...doc.data(), 'id': doc.id});
        await LocalDatabase.objects.put(obj.id, obj);
      }
    } catch (e) { print('Objects sync error: $e'); }
  }
}
...
class SelectionToolCard extends StatelessWidget {
  ...
  const SelectionToolCard({super.key, required this.tool, required this.selectionMode, required this.onTap});

  @override
  Widget build(BuildContext context) {
    return Consumer2<ToolsProvider, AuthProvider>(
      builder: (context, toolsProvider, authProvider, child) {
        return Card(margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), elevation: 4, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
          child: InkWell(
            onTap: selectionMode ? () => toolsProvider.toggleToolSelection(tool.id) : onTap,
            onLongPress: () { if (!selectionMode) { toolsProvider.toggleSelectionMode(); toolsProvider.toggleToolSelection(tool.id); } },
            borderRadius: BorderRadius.circular(16),
            child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.white, Colors.grey.shade50]), borderRadius: BorderRadius.circular(16)),
              child: Row(children: [
                if (selectionMode) Padding(padding: const EdgeInsets.only(right: 8), child: Checkbox(value: tool.isSelected, onChanged: (value) => toolsProvider.toggleToolSelection(tool.id), shape: const CircleBorder())),
                Container(width: 60, height: 60, decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Theme.of(context).colorScheme.primary.withOpacity(0.1), Theme.of(context).colorScheme.primary.withOpacity(0.05)])),
                  child: tool.displayImage != null
                    ? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image(image: tool.displayImage!.startsWith('http') ? NetworkImage(tool.displayImage!) as ImageProvider : FileImage(File(tool.displayImage!)), fit: BoxFit.cover, errorBuilder: (_,__,___) => Center(child: Icon(Icons.build, color: Theme.of(context).colorScheme.primary, size: 30))))
                    : Center(child: Icon(Icons.build, color: Theme.of(context).colorScheme.primary, size: 30)),
                ),
                const SizedBox(width: 12),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Expanded(child: Text(tool.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis)),
                    if (!selectionMode) IconButton(icon: Icon(tool.isFavorite ? Icons.favorite : Icons.favorite_border, color: tool.isFavorite ? Colors.red : Colors.grey, size: 20), onPressed: () => toolsProvider.toggleFavorite(tool.id), padding: EdgeInsets.zero, constraints: const BoxConstraints()),
                  ]),
                  const SizedBox(height: 4), Text(tool.brand, style: TextStyle(fontSize: 14, color: Colors.grey[600])), const SizedBox(height: 4),
                  Row(children: [const Icon(Icons.location_on, size: 12, color: Colors.grey), const SizedBox(width: 4), Expanded(child: Text(tool.currentLocationName, style: const TextStyle(fontSize: 12, color: Colors.grey), maxLines: 1, overflow: TextOverflow.ellipsis))]),
                ])),
                if (!selectionMode) PopupMenuButton(
                  itemBuilder: (context) {
                    List<PopupMenuItem> items = [];
                    if (authProvider.isAdmin) {
                      items.add(PopupMenuItem(child: ListTile(leading: const Icon(Icons.edit), title: const Text('Редактировать'), onTap: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (context) => AddEditToolScreen(tool: tool))); })));
                    }
                    if (tool.currentLocation == 'garage' && authProvider.isAdmin) {
                      items.add(PopupMenuItem(child: ListTile(leading: const Icon(Icons.copy), title: const Text('Дублировать'), onTap: () { Navigator.pop(context); toolsProvider.duplicateTool(tool); })));
                    }
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, builder: (context) {
      return StatefulBuilder(builder: (context, setState) {
        return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(borderRadius:  BorderRadius.vertical(top: Radius.circular(20)), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.white, Colors.grey.shade100] ,)), child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const Divider(),
            ...objectsProvider.objects.map((obj) => ListTile(leading: const Icon(Icons.location_city, color: Colors.orange), title: Text(obj.name), subtitle: Text('${obj.toolIds.length} инструментов'), trailing: selectedId == obj.id ? const Icon(Icons.check, color: Colors.green) : null, onTap: () { setState(() { selectedId = obj.id; selectedName = obj.name; }); })),
            const SizedBox(height: 20),
            Row(children: [
  ...
  void _showDeleteDialog(BuildContext context, Tool tool) {
    ...
    showDialog(context: context, builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        TextButton(onPressed: () async { Navigator.pop(context); await Provider.of<ToolsProvider>(context, listen: false).deleteTool(tool.id); }, child: const Text('Удалить', style: TextStyle(color: Colors.red))),
      ],
    ));
  }
}
...
class _AddEditToolScreenState extends State<AddEditToolScreen> {
  ...
  File? _imageFile; bool _isLoading = false; String? _imageUrl; String? _localImagePath;

  @override
  void initState() { super.initState(); if (widget.tool != null) { _titleController.text = widget.tool!.title; _descriptionController.text = widget.tool!.description; _brandController.text = widget.tool!.brand; _uniqueIdController.text = widget.tool!.uniqueId; _imageUrl = widget.tool!.imageUrl; _localImagePath = widget.tool!.localImagePath; } else { _uniqueIdController.text = IdGenerator.generateUniqueId(); } }
  @override void dispose() { _titleController.dispose(); _descriptionController.dispose(); _brandController.dispose(); _uniqueIdController.dispose(); super.dispose(); }
  Future<void> _pickImage() async { final file = await ImageService.pickImage(); if (file != null) setState(() { _imageFile = file; _imageUrl = null; _localImagePath = null; }); }
  Future<void> _takePhoto() async { final file = await ImageService.takePhoto(); if (file != null) setState(() { _imageFile = file; _imageUrl = null; _localImagePath = null; }); }
  Future<void> _saveTool() async {
    ...
    try {
      ...
      if (widget.tool == null) await toolsProvider.addTool(tool, imageFile: _imageFile);
      else await toolsProvider.updateTool(tool, imageFile: _imageFile);
      Navigator.pop(context);
    } catch (e) { ErrorHandler.showErrorDialog(context, 'Ошибка сохранения: $e'); } finally { setState(() => _isLoading = false); }
  }
  @override
  Widget build(BuildContext context) {
    ...
    if (!auth.isAdmin) {
      return Scaffold(body: Center(child: Text('Только администратор может редактировать инструменты')));
    }
    return Scaffold(
      appBar: AppBar(title: Text(isEdit ? 'Редактировать инструмент' : 'Добавить инструмент'), actions: [if (isEdit && auth.isAdmin) IconButton(icon: const Icon(Icons.delete), onPressed: () { showDialog(context: context, builder: (context) => AlertDialog(title: const Text('Подтверждение удаления'), content: Text('Удалить "${widget.tool!.title}"?'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Отмена')), TextButton(onPressed: () async { Navigator.pop(context); await Provider.of<ToolsProvider>(context, listen: false).deleteTool(widget.tool!.id); Navigator.pop(context); }, child: const Text('Удалить', style: TextStyle(color: Colors.red)))])); }) ]),
      body: _isLoading ? const Center(child: CircularProgressIndicator()) : SingleChildScrollView(padding: const EdgeInsets.all(16), child: Form(key: _formKey, child: Column(children: [
        ...
        const SizedBox(height: 20),
        TextFormField(controller: _titleController, decoration: InputDecoration(labelText: 'Название инструмента *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), prefixIcon: const Icon(Icons.title), filled: true, fillColor: Colors.grey.shade50), validator: (v) => v?.isEmpty == true ? 'Введите название' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _brandController, decoration: InputDecoration(labelText: 'Бренд *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), prefixIcon: const Icon(Icons.branding_watermark), filled: true, fillColor: Colors.grey.shade50), validator: (v) => v?.isEmpty == true ? 'Введите бренд' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _uniqueIdController, decoration: InputDecoration(labelText: 'Уникальный идентификатор *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), prefixIcon: const Icon(Icons.qr_code), suffixIcon: IconButton(icon: const Icon(Icons.refresh), onPressed: () => _uniqueIdController.text = IdGenerator.generateUniqueId()), filled: true, fillColor: Colors.grey.shade50), validator: (v) => v?.isEmpty == true ? 'Введите идентификатор' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _descriptionController, decoration: InputDecoration(labelText: 'Описание', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), alignLabelWithHint: true, prefixIcon: const Icon(Icons.description), filled: true, fillColor: Colors.grey.shade50), maxLines: 4),
        const SizedBox(height: 32),
        ElevatedButton(onPressed: _saveTool, style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: Text(isEdit ? 'Сохранить изменения' : 'Добавить инструмент', style: const TextStyle(fontSize: 16))),
        const SizedBox(height: 20),
      ]))),
    );
  }
  Widget _getImageWidget() {
    if (_imageFile != null) return ClipRRect(borderRadius: BorderRadius.circular(16), child: Image.file(_imageFile!, fit: BoxFit.cover));
    if (_imageUrl != null) return ClipRRect(borderRadius: BorderRadius.circular(16), child: Image.network(_imageUrl!, fit: BoxFit.cover, errorBuilder: (_,__,___) => _buildPlaceholder()));
    if (_localImagePath != null) return ClipRRect(borderRadius: BorderRadius.circular(16), child: Image.file(File(_localImagePath!), fit: BoxFit.cover, errorBuilder: (_,__,___) => _buildPlaceholder()));
    return _buildPlaceholder();
  }
  Widget _buildPlaceholder() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.camera_alt, size: 50, color: Colors.grey.shade400), const SizedBox(height: 8), Text('Добавить фото инструмента', style: TextStyle(color: Colors.grey.shade600))]));
  void _showImagePickerDialog() {
    showModalBottomSheet(context: context, builder: (context) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ListTile(leading: const Icon(Icons.photo_library), title: const Text('Выбрать из галереи'), onTap: () { Navigator.pop(context); _pickImage(); }),
      ListTile(leading: const Icon(Icons.camera_alt), title: const Text('Сделать фото'), onTap: () { Navigator.pop(context); _takePhoto(); }),
      if (_imageFile != null || _imageUrl != null || _localImagePath != null) ListTile(leading: const Icon(Icons.delete, color: Colors.red), title: const Text('Удалить фото', style: TextStyle(color: Colors.red)), onTap: () { Navigator.pop(context); setState(() { _imageFile = null; _imageUrl = null; _localImagePath = null; }); }),
    ])));
  }
}
...
class _EnhancedGarageScreenState extends State<EnhancedGarageScreen> {
  @override void initState() { super.initState(); WidgetsBinding.instance.addPostFrameCallback((_) { Provider.of<ToolsProvider>(context, listen: false).loadTools(); }); }
  @override Widget build(BuildContext context) {
    final toolsProvider = Provider.of<ToolsProvider>(context);
    final authProvider = Provider.of<AuthProvider>(context);
    final garageTools = toolsProvider.garageTools;
    return Scaffold(
      body: toolsProvider.isLoading && garageTools.isEmpty ? _buildLoadingScreen() : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(width: double.infinity, padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Theme.of(context).colorScheme.primary, Theme.of(context).colorScheme.secondary]), borderRadius: const BorderRadius.only(bottomLeft: Radius.circular(30), bottomRight: Radius.circular(30))),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Мой Гараж', style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Colors.white)),
            const SizedBox(height: 8), Text('${garageTools.length} инструментов доступно', style: const TextStyle(fontSize: 16, color: Colors.white70)),
            const SizedBox(height: 16),
            SingleChildScrollView(scrollDirection: Axis.horizontal, child: SizedBox(width: MediaQuery.of(context).size.width, child: Row(mainAxisSize: MainAxisSize.max, mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
              _buildStatCard(context, '    Всего    ', '${toolsProvider.totalTools}', Icons.build, Colors.white.withOpacity(0.2)),
              const SizedBox(width: 10), _buildStatCard(context, 'В гараже', '${garageTools.length}', Icons.garage, Colors.white.withOpacity(0.2)),
              const SizedBox(width: 10), _buildStatCard(context, 'Избранные', '${toolsProvider.favoriteTools.length}', Icons.favorite, Colors.white.withOpacity(0.2)),
            ]))),
          ]),
        ),
        Padding(padding: const EdgeInsets.all(16), child: Row(children: [
          if (authProvider.isAdmin)
            Expanded(child: ElevatedButton.icon(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const AddEditToolScreen())), icon: const Icon(Icons.add), label: const Text('Добавить'), style: ElevatedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))))),
          if (authProvider.isAdmin) const SizedBox(width: 8),
          ElevatedButton.icon(onPressed: toolsProvider.toggleSelectionMode, icon: const Icon(Icons.checklist), label: Text(toolsProvider.selectionMode ? 'Отменить' : 'Выбрать'), style: ElevatedButton.styleFrom(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))),
        ])),
        Expanded(child: garageTools.isEmpty ? _buildEmptyGarage(authProvider.isAdmin) : ListView.builder(padding: const EdgeInsets.all(8), itemCount: garageTools.length, itemBuilder: (context, index) {
          final tool = garageTools[index];
          return SelectionToolCard(tool: tool, selectionMode: toolsProvider.selectionMode, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => EnhancedToolDetailsScreen(tool: tool))));
        })),
      ]),
      floatingActionButton: toolsProvider.selectionMode && toolsProvider.hasSelectedTools ? FloatingActionButton.extended(onPressed: () => _showGarageSelectionActions(context), icon: const Icon(Icons.more_vert), label: Text('${toolsProvider.selectedTools.length}'), backgroundColor: Theme.of(context).colorScheme.primary) : null,
      floatingActionButtonLocation: FloatingActionButtonLocation.centerFloat,
    );
  }
  Widget _buildLoadingScreen() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Theme.of(context).colorScheme.primary)), const SizedBox(height: 20), Text('Загрузка гаража...', style: TextStyle(fontSize: 16, color: Colors.grey[600]))]));
  Widget _buildStatCard(BuildContext context, String title, String value, IconData icon, Color backgroundColor) => Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: BoxDecoration(color: backgroundColor, borderRadius: BorderRadius.circular(12)), child: Column(children: [Icon(icon, size: 24, color: Colors.white), const SizedBox(height: 4), Text(value, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)), Text(title, style: const TextStyle(fontSize: 12, color: Colors.white70))]));
  Widget _buildEmptyGarage(bool isAdmin) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.garage, size: 80, color: Colors.grey.shade300), const SizedBox(height: 20), Text('Гараж пуст', style: TextStyle(fontSize: 18, color: Colors.grey[600])), const SizedBox(height: 10), Text('Добавьте инструменты в гараж', style: TextStyle(color: Colors.grey[500])), const SizedBox(height: 20), if (isAdmin) ElevatedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const AddEditToolScreen())), child: const Text('Добавить первый инструмент'))]));
  ...
  void _showMultiDeleteDialog(BuildContext context) {
    ...
    showDialog(context: context, builder: (context) => AlertDialog(
      ...
      content: Text('Удалить выбранные ${toolsProvider.selectedTools.length} инструментов?'),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Отмена')),
        TextButton(onPressed: () async { Navigator.pop(context); await toolsProvider.deleteSelectedTools(); }, child: const Text('Удалить', style: TextStyle(color: Colors.red))),
      ],
    ));
  }
...
class EnhancedToolDetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: CustomScrollView(slivers: [
        SliverAppBar(expandedHeight: 300, pinned: true, flexibleSpace: FlexibleSpaceBar(background: Hero(tag: 'tool-${tool.id}', child: tool.displayImage != null
          ? Image(image: tool.displayImage!.startsWith('http') ? NetworkImage(tool.displayImage!) as ImageProvider : FileImage(File(tool.displayImage!)), fit: BoxFit.cover)
          : Container(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [theme.colorScheme.primary.withOpacity(0.2), theme.colorScheme.secondary.withOpacity(0.2)])), child: Center(child: Icon(Icons.build, size: 100, color: theme.colorScheme.primary.withOpacity(0.5)))))),
          actions: [
            IconButton(icon: const Icon(Icons.share), onPressed: () => ReportService.showReportTypeDialog(context, tool, (type) => ReportService.shareToolReport(tool, context, type))),
            IconButton(icon: const Icon(Icons.print), onPressed: () => ReportService.printToolReport(tool, context)),
            PopupMenuButton(
              itemBuilder: (context) {
                List<PopupMenuItem> items = [];
                if (auth.isAdmin) {
                ...
                if (tool.currentLocation == 'garage' && auth.isAdmin) {
                  items.add(PopupMenuItem(child: ListTile(leading: const Icon(Icons.copy), title: const Text('Дублировать'), onTap: () { Navigator.pop(context); Provider.of<ToolsProvider>(context, listen: false).duplicateTool(tool); Navigator.pop(context); })));
                }
                if (auth.isAdmin) {
                  items.add(PopupMenuItem(child: ListTile(leading: const Icon(Icons.delete, color: Colors.red), title: const Text('Удалить', style: TextStyle(color: Colors.red)), onTap: () => _showDeleteConfirmation(context))));
                }
                return items;
              },
            ),
        ...
        SliverToBoxAdapter(child: Padding(padding: const EdgeInsets.all(20), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Text(tool.title, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold))),
            Consumer<ToolsProvider>(builder: (context, tp, _) => IconButton(icon: Icon(tool.isFavorite ? Icons.favorite : Icons.favorite_border, color: tool.isFavorite ? Colors.red : null, size: 30), onPressed: () => tp.toggleFavorite(tool.id))) ]),
          const SizedBox(height: 10),
          Row(children: [
            Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [theme.colorScheme.primary.withOpacity(0.1), theme.colorScheme.secondary.withOpacity(0.1)]), borderRadius: BorderRadius.circular(20)), child: Text(tool.brand, style: TextStyle(color: theme.colorScheme.primary, fontWeight: FontWeight.w500))),
            const SizedBox(width: 10), const Icon(Icons.qr_code, size: 16, color: Colors.grey), const SizedBox(width: 5), Text(tool.uniqueId, style: const TextStyle(color: Colors.grey)),
          ]),
          const SizedBox(height: 20),
          if (tool.description.isNotEmpty) Card(elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Описание', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: theme.colorScheme.primary)), const SizedBox(height: 10), Text(tool.description, style: TextStyle(fontSize: 16, height: 1.5, color: Colors.grey[700])),
          ]))),
          const SizedBox(height: 20),
          GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 2, childAspectRatio: 1.8, crossAxisSpacing: 10, mainAxisSpacing: 10, children: [
            _buildDetailCard(icon: Icons.location_on, title: 'Местоположение', value: tool.currentLocationName, color: Colors.blue),
            _buildDetailCard(icon: Icons.calendar_today, title: 'Добавлен', value: DateFormat('dd.MM.yyyy').format(tool.createdAt), color: Colors.green),
            _buildDetailCard(icon: Icons.update, title: 'Обновлен', value: DateFormat('dd.MM.yyyy').format(tool.updatedAt), color: Colors.orange),
            _buildDetailCard(icon: Icons.star, title: 'Статус', value: tool.isFavorite ? 'Избранный' : 'Обычный', color: Colors.purple),
          ]),
          const SizedBox(height: 20),
          if (tool.locationHistory.isNotEmpty) Card(elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [const Icon(Icons.history, color: Colors.purple), const SizedBox(width: 10), Text('История перемещений', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: theme.colorScheme.primary))]),
            const SizedBox(height: 10),
            ...tool.locationHistory.map((history) => Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Row(children: [Container(width: 8, height: 8, decoration: const BoxDecoration(color: Colors.purple, shape: BoxShape.circle)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(history.locationName, style: const TextStyle(fontWeight: FontWeight.w500)), const SizedBox(height: 2), Text(DateFormat('dd.MM.yyyy HH:mm').format(history.date), style: const TextStyle(fontSize: 12, color: Colors.grey))]))]))),
          ]))),
  ...
  Widget _buildDetailCard({required IconData icon, required String title, required String value, required Color color}) => Card(elevation: 2, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
    Row(children: [Icon(icon, size: 20, color: color), const SizedBox(width: 8), Text(title, style: TextStyle(fontSize: 12, color: Colors.grey[600], fontWeight: FontWeight.w500))]), const SizedBox(height: 6), Text(value, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.black87)),
  ])));
...
class ObjectCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final toolsOnObject = toolsProvider.tools.where((tool) => tool.currentLocation == object.id).toList();
    final auth = Provider.of<AuthProvider>(context);
    return Consumer<ObjectsProvider>(builder: (context, objectsProvider, child) {
      return Card(margin: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), elevation: 4, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        child: InkWell(
          onTap: selectionMode ? () => objectsProvider.toggleObjectSelection(object.id) : onTap,
          onLongPress: () { if (!selectionMode) { objectsProvider.toggleSelectionMode(); objectsProvider.toggleObjectSelection(object.id); } },
          borderRadius: BorderRadius.circular(16),
          child: Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.white, Colors.grey.shade50]), borderRadius: BorderRadius.circular(16)),
            child: Row(children: [
              if (selectionMode) Padding(padding: const EdgeInsets.only(right: 8), child: Checkbox(value: object.isSelected, onChanged: (value) => objectsProvider.toggleObjectSelection(object.id), shape: const CircleBorder())),
              Container(width: 60, height: 60, decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.orange.withOpacity(0.1), Colors.orange.withOpacity(0.05)])),
                child: object.displayImage != null
                  ? ClipRRect(borderRadius: BorderRadius.circular(12), child: Image(image: object.displayImage!.startsWith('http') ? NetworkImage(object.displayImage!) as ImageProvider : FileImage(File(object.displayImage!)), fit: BoxFit.cover, errorBuilder: (_,__,___) => Center(child: Icon(Icons.location_city, color: Colors.orange, size: 30))))
                  : Center(child: Icon(Icons.location_city, color: Colors.orange, size: 30)),
              ),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(object.name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis),
                if (object.description.isNotEmpty) const SizedBox(height: 4),
                if (object.description.isNotEmpty) Text(object.description, style: TextStyle(fontSize: 14, color: Colors.grey[600]), maxLines: 2, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 4),
                Row(children: [const Icon(Icons.build, size: 12, color: Colors.grey), const SizedBox(width: 4), Text('${toolsOnObject.length} инструментов', style: const TextStyle(fontSize: 12, color: Colors.grey))]),
              ])),
              if (!selectionMode) PopupMenuButton(
                itemBuilder: (context) {
                  List<PopupMenuItem> items = [];
...
class AddEditObjectScreen extends StatefulWidget {
  ...
  @override _AddEditObjectScreenState createState() => _AddEditObjectScreenState();
}
class _AddEditObjectScreenState extends State<AddEditObjectScreen> {
  ...
  final _descriptionController = TextEditingController();
  File? _imageFile; bool _isLoading = false; String? _imageUrl; String? _localImagePath;
  @override void initState() { super.initState(); if (widget.object != null) { _nameController.text = widget.object!.name; _descriptionController.text = widget.object!.description; _imageUrl = widget.object!.imageUrl; _localImagePath = widget.object!.localImagePath; } }
  @override void dispose() { _nameController.dispose(); _descriptionController.dispose(); super.dispose(); }
  Future<void> _pickImage() async { final file = await ImageService.pickImage(); if (file != null) setState(() { _imageFile = file; _imageUrl = null; _localImagePath = null; }); }
  Future<void> _takePhoto() async { final file = await ImageService.takePhoto(); if (file != null) setState(() { _imageFile = file; _imageUrl = null; _localImagePath = null; }); }
  Future<void> _saveObject() async {
    ...
    try {
      ...
      if (widget.object == null) await objectsProvider.addObject(object, imageFile: _imageFile);
      else await objectsProvider.updateObject(object, imageFile: _imageFile);
      Navigator.pop(context);
    } catch (e) { ErrorHandler.showErrorDialog(context, 'Ошибка сохранения: $e'); } finally { setState(() => _isLoading = false); }
  }
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isLoading ? const Center(child: CircularProgressIndicator()) : SingleChildScrollView(padding: const EdgeInsets.all(16), child: Form(key: _formKey, child: Column(children: [
        GestureDetector(onTap: _showImagePickerDialog, child: Container(height: 200, width: double.infinity, decoration: BoxDecoration(borderRadius: BorderRadius.circular(16), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Theme.of(context).colorScheme.primary.withOpacity(0.1), Theme.of(context).colorScheme.secondary.withOpacity(0.1)]), border: Border.all(color: Colors.grey.shade300)), child: _getImageWidget())),
        const SizedBox(height: 20),
        TextFormField(controller: _nameController, decoration: InputDecoration(labelText: 'Название объекта *', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), prefixIcon: const Icon(Icons.title), filled: true, fillColor: Colors.grey.shade50), validator: (v) => v?.isEmpty == true ? 'Введите название' : null),
        const SizedBox(height: 16),
        TextFormField(controller: _descriptionController, decoration: InputDecoration(labelText: 'Описание', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), alignLabelWithHint: true, prefixIcon: const Icon(Icons.description), filled: true, fillColor: Colors.grey.shade50), maxLines: 4),
        const SizedBox(height: 32),
        ElevatedButton(onPressed: _saveObject, style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: Text(isEdit ? 'Сохранить изменения' : 'Добавить объект', style: const TextStyle(fontSize: 16))),
        const SizedBox(height: 20),
      ]))),
    );
  }
  ...
  Widget _buildPlaceholder() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.location_city, size: 50, color: Colors.grey.shade400), const SizedBox(height: 8), Text('Добавить фото объекта', style: TextStyle(color: Colors.grey.shade600))]));
  void _showImagePickerDialog() {
    showModalBottomSheet(context: context, builder: (context) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ListTile(leading: const Icon(Icons.photo_library), title: const Text('Выбрать из галереи'), onTap: () { Navigator.pop(context); _pickImage(); }),
      ListTile(leading: const Icon(Icons.camera_alt), title: const Text('Сделать фото'), onTap: () { Navigator.pop(context); _takePhoto(); }),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        Container(height: 200, width: double.infinity, decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.grey.shade100, Colors.grey.shade200])),
          child: object.displayImage != null
            ? Image(image: object.displayImage!.startsWith('http') ? NetworkImage(object.displayImage!) as ImageProvider : FileImage(File(object.displayImage!)), fit: BoxFit.cover)
            : Center(child: Icon(Icons.location_city, size: 80, color: Colors.grey.shade300)),
        ),
        Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(object.name, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          if (object.description.isNotEmpty) Text(object.description, style: const TextStyle(fontSize: 16, color: Colors.grey)),
          const SizedBox(height: 16),
          Row(children: [const Icon(Icons.build, color: Colors.grey), const SizedBox(width: 8), Text('Инструментов на объекте: ${toolsOnObject.length}', style: const TextStyle(fontSize: 16))]),
          const SizedBox(height: 8),
          Row(children: [const Icon(Icons.calendar_today, color: Colors.grey), const SizedBox(width: 8), Text('Создан: ${DateFormat('dd.MM.yyyy').format(object.createdAt)}', style: const TextStyle(fontSize: 14, color: Colors.grey))]),
        ])),
        const Divider(),
        Expanded(child: toolsOnObject.isEmpty ? _buildEmptyObjectTools() : ListView.builder(itemCount: toolsOnObject.length, itemBuilder: (context, index) {
          final tool = toolsOnObject[index];
          return SelectionToolCard(tool: tool, selectionMode: false, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => EnhancedToolDetailsScreen(tool: tool))));
        })),
      ]),
    );
  }
...
class _EnhancedObjectsListScreenState extends State<EnhancedObjectsListScreen> {
  ...
  @override void initState() { super.initState(); WidgetsBinding.instance.addPostFrameCallback((_) { Provider.of<ObjectsProvider>(context, listen: false).loadObjects(); }); }
  ...
  void _showObjectsDeleteDialog(BuildContext context) {
    final objectsProvider = Provider.of<ObjectsProvider>(context, listen: false);
    showDialog(context: context, builder: (context) => AlertDialog(title: const Text('Подтверждение удаления'), content: Text('Удалить выбранные ${objectsProvider.selectedObjects.length} объектов?\n\nИнструменты на этих объектах будут перемещены в гараж.'), actions: [
      TextButton(onPressed: () => Navigator.pop(context), child: const Text('Отмена')),
      TextButton(onPressed: () async { Navigator.pop(context); await objectsProvider.deleteSelectedObjects(); }, child: const Text('Удалить', style: TextStyle(color: Colors.red))),
    ]));
  }
}
...
class MoveToolsScreen extends StatefulWidget {
  ...
  @override _MoveToolsScreenState createState() => _MoveToolsScreenState();
}
class _MoveToolsScreenState extends State<MoveToolsScreen> {
  String? _selectedLocationId; String? _selectedLocationName;
  @override Widget build(BuildContext context) {
    final toolsProvider = Provider.of<ToolsProvider>(context);
    final objectsProvider = Provider.of<ObjectsProvider>(context);
    return Scaffold(
      ...
      body: Column(children: [
        Card(margin: const EdgeInsets.all(16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Выберите место назначения:', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)), const SizedBox(height: 12),
          ListTile(leading: const Icon(Icons.garage, color: Colors.blue), title: const Text('Гараж'), trailing: _selectedLocationId == 'garage' ? const Icon(Icons.check, color: Colors.green) : null, onTap: () { setState(() { _selectedLocationId = 'garage'; _selectedLocationName = 'Гараж'; }); }),
          const Divider(),
          ...objectsProvider.objects.map((obj) => ListTile(leading: const Icon(Icons.location_city, color: Colors.orange), title: Text(obj.name), subtitle: Text('${obj.toolIds.length} инструментов'), trailing: _selectedLocationId == obj.id ? const Icon(Icons.check, color: Colors.green) : null, onTap: () { setState(() { _selectedLocationId = obj.id; _selectedLocationName = obj.name; }); })),
        ]))),
        Expanded(child: ListView.builder(itemCount: widget.selectedTools.length, itemBuilder: (context, index) {
          final tool = widget.selectedTools[index];
          return Card(margin: const EdgeInsets.symmetric(horizontal: 16, vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: ListTile(
            leading: CircleAvatar(backgroundColor: Theme.of(context).colorScheme.primary.withOpacity(0.1), child: Icon(Icons.build, color: Theme.of(context).colorScheme.primary)),
            title: Text(tool.title), subtitle: Text(tool.brand), trailing: Text(tool.currentLocationName, style: const TextStyle(color: Colors.grey)),
          ));
        })),
        Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, border: Border(top: BorderSide(color: Colors.grey.shade200))),
          child: ElevatedButton(onPressed: () async {
            if (_selectedLocationId == null || _selectedLocationName == null) { ErrorHandler.showWarningDialog(context, 'Выберите место назначения'); return; }
            await toolsProvider.moveSelectedTools(_selectedLocationId!, _selectedLocationName!);
            Navigator.pop(context);
          }, style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: Text('Переместить ${widget.selectedTools.length} инструментов')),
        ),
      ]),
    );
  }
...
class FavoritesScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: Text('Избранное (${favoriteTools.length})')),
      body: favoriteTools.isEmpty ? _buildEmptyFavoritesScreen() : ListView.builder(itemCount: favoriteTools.length, itemBuilder: (context, index) {
...
class AdminMoveRequestsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: pending.isEmpty ? Center(child: Text('Нет ожидающих запросов')) : ListView.builder(itemCount: pending.length, itemBuilder: (context, index) {
        ...
        return Card(margin: const EdgeInsets.all(8), child: ListTile(
          ...
          subtitle: Text('${req.fromLocationName} → ${req.toLocationName} от ${req.requestedBy}'),
          trailing: Row(mainAxisSize: MainAxisSize.min, children: [
            IconButton(icon: const Icon(Icons.check, color: Colors.green), onPressed: () async {
              await toolsProvider.moveTool(req.toolId, req.toLocationId, req.toLocationName);
...
class NotificationsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final notifProvider = Provider.of<NotificationProvider>(context);
    return Scaffold(
      appBar: AppBar(title: const Text('Уведомления'), actions: [
        if (notifProvider.hasUnread)
          TextButton(onPressed: () => notifProvider.markAllRead(), child: const Text('Прочитать все'))
      ]),
      body: notifProvider.notifications.isEmpty
        ? Center(child: Text('Нет уведомлений'))
        : ListView.builder(itemCount: notifProvider.notifications.length, itemBuilder: (context, index) {
            final notif = notifProvider.notifications[index];
            return ListTile(
              leading: Icon(notif.read ? Icons.notifications_none : Icons.notifications_active, color: notif.read ? Colors.grey : Colors.blue),
...
// ========== PROFILE SCREEN (updated with admin panel link) ==========
class ProfileScreen extends StatefulWidget {
  const ProfileScreen({super.key});
  @override _ProfileScreenState createState() => _ProfileScreenState();
}
class _ProfileScreenState extends State<ProfileScreen> {
  bool _syncEnabled = true; bool _notificationsEnabled = true; String _themeMode = 'light';
  @override void initState() { super.initState(); _loadSettings(); }
  Future<void> _loadSettings() async { final prefs = await SharedPreferences.getInstance(); setState(() { _syncEnabled = prefs.getBool('sync_enabled') ?? true; _notificationsEnabled = prefs.getBool('notifications_enabled') ?? true; _themeMode = prefs.getString('theme_mode') ?? 'light'; }); }
  Future<void> _saveSetting(String key, dynamic value) async { final prefs = await SharedPreferences.getInstance(); if (value is bool) prefs.setBool(key, value); else if (value is String) prefs.setString(key, value); }
  Future<void> _changeTheme(String mode) async { setState(() => _themeMode = mode); await _saveSetting('theme_mode', mode); if (mounted) { final context = navigatorKey.currentContext; if (context != null) { final myApp = MyApp(); runApp(myApp); } } }
  @override Widget build(BuildContext context) {
    final authProvider = Provider.of<AuthProvider>(context);
    final toolsProvider = Provider.of<ToolsProvider>(context);
    final objectsProvider = Provider.of<ObjectsProvider>(context);
    ...
    return Scaffold(
      body: SingleChildScrollView(child: Column(children: [
        Container(height: 250, decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Theme.of(context).colorScheme.primary, Theme.of(context).colorScheme.secondary])),
          child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            Stack(children: [
              CircleAvatar(radius: 60, backgroundColor: Colors.white, backgroundImage: authProvider.profileImage != null ? FileImage(authProvider.profileImage!) : null, child: authProvider.profileImage == null ? Icon(Icons.person, size: 60, color: Theme.of(context).colorScheme.primary) : null),
              Positioned(bottom: 0, right: 0, child: CircleAvatar(radius: 20, backgroundColor: Colors.white, child: IconButton(icon: Icon(Icons.camera_alt, size: 15, color: Theme.of(context).colorScheme.primary), onPressed: () => _pickProfileImage(authProvider), padding: EdgeInsets.zero))),
            ]),
            const SizedBox(height: 20),
            Text(authProvider.user?.email ?? 'Гость', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white)),
            const SizedBox(height: 4), const Text('Менеджер инструментов', style: TextStyle(color: Colors.white70)),
          ])),
        ),
        Padding(padding: const EdgeInsets.all(16), child: GridView.count(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisCount: 2, childAspectRatio: 1, crossAxisSpacing: 10, mainAxisSpacing: 10, children: [
          _buildStatCard('Всего инструментов', '${toolsProvider.totalTools}', Icons.build, Colors.blue),
          _buildStatCard('В гараже', '${toolsProvider.garageTools.length}', Icons.garage, Colors.green),
          _buildStatCard('Объектов', '${objectsProvider.totalObjects}', Icons.location_city, Colors.orange),
          _buildStatCard('Избранных', '${toolsProvider.favoriteTools.length}', Icons.favorite, Colors.red),
        ])),
        Card(margin: const EdgeInsets.all(16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), child: Padding(padding: const EdgeInsets.all(16), child: Column(children: [
          const Text('Настройки', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), const SizedBox(height: 16),
          ListTile(leading: const Icon(Icons.sync), title: const Text('Синхронизация данных'), trailing: Switch(value: _syncEnabled, onChanged: (v) { setState(() => _syncEnabled = v); _saveSetting('sync_enabled', v); })),
          ListTile(leading: const Icon(Icons.notifications), title: const Text('Уведомления'), trailing: Switch(value: _notificationsEnabled, onChanged: (v) { setState(() => _notificationsEnabled = v); _saveSetting('notifications_enabled', v); })),
        ...
        Padding(padding: const EdgeInsets.all(16), child: Column(children: [
          ...
          const SizedBox(height: 12),
          ElevatedButton.icon(onPressed: () async => await _createBackup(context, toolsProvider, objectsProvider), icon: const Icon(Icons.backup), label: const Text('Создать резервную копию'), style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 50), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))),
          const SizedBox(height: 12),
          OutlinedButton.icon(onPressed: () async { await authProvider.signOut(); }, icon: const Icon(Icons.logout), label: const Text('Выйти'), style: OutlinedButton.styleFrom(minimumSize: const Size(double.infinity, 50), side: const BorderSide(color: Colors.red), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))),
        ])),
      ])),
    );
  }
  Widget _buildStatCard(String title, String value, IconData icon, Color color) => Card(elevation: 3, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), child: Padding(padding: const EdgeInsets.all(16), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, color: color, size: 30), const SizedBox(height: 8), Text(value, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), const SizedBox(height: 4), Text(title, style: TextStyle(fontSize: 12, color: Colors.grey[600]), textAlign: TextAlign.center)])));
  Future<void> _pickProfileImage(AuthProvider auth) async { final file = await ImageService.pickImage(); if (file != null) { await auth.setProfileImage(file); ErrorHandler.showSuccessDialog(context, 'Фото профиля обновлено'); } }
  Future<void> _createBackup(BuildContext context, ToolsProvider tp, ObjectsProvider op) async {
    try {
      final backupData = {'tools': tp.tools.map((t) => t.toJson()).toList(), 'objects': op.objects.map((o) => o.toJson()).toList(), 'createdAt': DateTime.now().toIso8601String(), 'version': '1.0'};
      final jsonStr = jsonEncode(backupData);
      final tempDir = await getTemporaryDirectory();
      final file = File('${tempDir.path}/tooler_backup_${DateTime.now().millisecondsSinceEpoch}.json');
      await file.writeAsString(jsonStr);
      await Share.shareXFiles([XFile(file.path)], text: '📱 Резервная копия Tooler\n\n📅 Дата: ${DateFormat('dd.MM.yyyy HH:mm').format(DateTime.now())}\n🛠️ Инструментов: ${tp.tools.length}\n🏢 Объектов: ${op.objects.length}\n\n— Создано в Tooler App —');
      ErrorHandler.showSuccessDialog(context, 'Резервная копия создана');
    } catch (e) { ErrorHandler.showErrorDialog(context, 'Ошибка: $e'); }
  }
}
...
class _SearchScreenState extends State<SearchScreen> {
  final TextEditingController _searchController = TextEditingController(); List<Tool> _searchResults = [];
  ...
  void _onSearchChanged() {
    ...
    final toolsProvider = Provider.of<ToolsProvider>(context, listen: false);
    setState(() => _searchResults = toolsProvider.tools.where((t) => t.title.toLowerCase().contains(q) || t.brand.toLowerCase().contains(q) || t.uniqueId.toLowerCase().contains(q) || t.description.toLowerCase().contains(q) || t.currentLocationName.toLowerCase().contains(q)).toList());
  }
  @override Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: TextField(controller: _searchController, autofocus: true, decoration: const InputDecoration(hintText: 'Поиск инструментов...', border: InputBorder.none, hintStyle: TextStyle(color: Colors.white70)), style: const TextStyle(color: Colors.white)), actions: [IconButton(icon: const Icon(Icons.clear), onPressed: () { _searchController.clear(); setState(() => _searchResults = []); })]),
    body: _searchResults.isEmpty ? _buildEmptySearchScreen() : ListView.builder(itemCount: _searchResults.length, itemBuilder: (context, index) {
...
class WelcomeScreen extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Scaffold(
    body: Container(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Theme.of(context).colorScheme.primary, Theme.of(context).colorScheme.secondary])),
      child: Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 32), const Text('Добро пожаловать в Tooler!', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Colors.white), textAlign: TextAlign.center),
        const SizedBox(height: 16), const Text('Простая и эффективная система управления строительными инструментами', style: TextStyle(fontSize: 16, color: Colors.white70), textAlign: TextAlign.center),
        const SizedBox(height: 48), ElevatedButton(onPressed: onContinue, style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 40, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30))), child: const Text('Начать работу', style: TextStyle(fontSize: 18))),
      ]))),
    ),
  );
}
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  final List<Map<String, dynamic>> _pages = [
    {'title': 'Управление инструментами', 'description': 'Легко добавляйте, редактируйте и отслеживайте все ваши строительные инструменты', 'icon': Icons.build, 'color': Colors.blue},
    {'title': 'Работа с объектами', 'description': 'Создавайте объекты и перемещаете инструменты между гаражом и объектами', 'icon': Icons.location_city, 'color': Colors.orange},
    {'title': 'Отчеты и PDF', 'description': 'Создавайте подробные отчеты и делитесь ими с коллегами', 'icon': Icons.picture_as_pdf, 'color': Colors.green},
    {'title': 'Работа офлайн', 'description': 'Продолжайте работу даже без интернета, данные синхронизируются автоматически', 'icon': Icons.wifi_off, 'color': Colors.purple},
  ];
  @override void dispose() { _pageController.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) => Scaffold(
    body: SafeArea(child: Column(children: [
      Align(alignment: Alignment.topRight, child: TextButton(onPressed: () { widget.onComplete(); Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const AuthScreen())); }, child: Text('Пропустить', style: TextStyle(color: Theme.of(context).colorScheme.primary)))),
      Expanded(child: PageView.builder(controller: _pageController, itemCount: _pages.length, onPageChanged: (i) => setState(() => _currentPage = i), itemBuilder: (context, i) {
        final p = _pages[i];
        return Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Container(width: 150, height: 150, decoration: BoxDecoration(color: p['color'].withOpacity(0.1), shape: BoxShape.circle), child: Icon(p['icon'], size: 70, color: p['color'])),
          const SizedBox(height: 40), Text(p['title'], style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
          const SizedBox(height: 20), Text(p['description'], style: const TextStyle(fontSize: 16, color: Colors.grey, height: 1.5), textAlign: TextAlign.center),
        ]));
      })),
      Padding(padding: const EdgeInsets.all(32), child: Row(children: [
        ...List.generate(_pages.length, (i) => Container(margin: const EdgeInsets.symmetric(horizontal: 4), width: 8, height: 8, decoration: BoxDecoration(shape: BoxShape.circle, color: _currentPage == i ? Theme.of(context).colorScheme.primary : Colors.grey[300]))),
        const Spacer(),
        ElevatedButton(onPressed: () { if (_currentPage < _pages.length - 1) { _pageController.nextPage(duration: const Duration(milliseconds: 300), curve: Curves.easeInOut); } else { widget.onComplete(); Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => const AuthScreen())); } }, child: Text(_currentPage == _pages.length - 1 ? 'Начать' : 'Далее')),
      ])),
    ])),
  );
}
...
class _AuthScreenState extends State<AuthScreen> {
  ...
  bool _isLogin = true; bool _isLoading = false; bool _obscurePassword = true; bool _obscureConfirmPassword = true; File? _profileImage;

  @override
  void initState() { super.initState(); _loadSavedEmail(); }
  Future<void> _loadSavedEmail() async { final prefs = await SharedPreferences.getInstance(); setState(() { _emailController.text = prefs.getString('saved_email') ?? ''; }); }
  Future<void> _pickProfileImage() async { final file = await ImageService.pickImage(); if (file != null) setState(() => _profileImage = file); }
  ...
  Future<void> _showForgotPasswordDialog() async {
    ...
    return showDialog(context: context, builder: (context) => AlertDialog(
      ...
      content: TextField(controller: emailController, decoration: const InputDecoration(labelText: 'Email', hintText: 'Введите ваш email'), keyboardType: TextInputType.emailAddress),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Отмена')),
  ...
  Future<void> _submit() async {
    ...
    try {
      final auth = Provider.of<AuthProvider>(context, listen: false);
      if (!_isLogin && _passwordController.text != _confirmPasswordController.text) throw Exception('Пароли не совпадают');
  ...
  Widget build(BuildContext context) {
    final authProvider = Provider.of<AuthProvider>(context);
    return Scaffold(
      body: SingleChildScrollView(child: Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        const SizedBox(height: 40),
        Center(child: Column(children: [
          if (!_isLogin && _profileImage != null) Stack(children: [
            CircleAvatar(radius: 60, backgroundImage: FileImage(_profileImage!)),
            Positioned(bottom: 0, right: 0, child: CircleAvatar(radius: 20, backgroundColor: Theme.of(context).colorScheme.primary, child: IconButton(icon: Icon(Icons.camera_alt, size: 15, color: Colors.white), onPressed: _pickProfileImage, padding: EdgeInsets.zero))),
          ]) else if (!_isLogin) GestureDetector(onTap: _pickProfileImage, child: CircleAvatar(radius: 60, backgroundColor: Theme.of(context).colorScheme.primary.withOpacity(0.1), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.camera_alt, size: 30, color: Theme.of(context).colorScheme.primary), const SizedBox(height: 8), Text('Фото', style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.primary))]))) else Container(width: 100, height: 100, decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Theme.of(context).colorScheme.primary, Theme.of(context).colorScheme.secondary]), shape: BoxShape.circle), child: const Icon(Icons.build, size: 60, color: Colors.white)),
          const SizedBox(height: 10), Text('Tooler', style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary)), const SizedBox(height: 5), const Text('Управление строительными инструментами', style: TextStyle(fontSize: 14, color: Colors.grey)),
        ])),
        const SizedBox(height: 40),
        Form(key: _formKey, child: Column(children: [
          TextFormField(controller: _emailController, decoration: InputDecoration(labelText: 'Email', prefixIcon: const Icon(Icons.email), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), filled: true, fillColor: Colors.grey.shade50), keyboardType: TextInputType.emailAddress, validator: (v) => v?.isEmpty == true ? 'Введите email' : v!.contains('@') ? null : 'Введите корректный email'),
          const SizedBox(height: 16),
          TextFormField(controller: _passwordController, decoration: InputDecoration(labelText: 'Пароль', prefixIcon: const Icon(Icons.lock), suffixIcon: IconButton(icon: Icon(_obscurePassword ? Icons.visibility : Icons.visibility_off), onPressed: () => setState(() => _obscurePassword = !_obscurePassword)), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), filled: true, fillColor: Colors.grey.shade50), obscureText: _obscurePassword, validator: (v) => v?.isEmpty == true ? 'Введите пароль' : v!.length >= 6 ? null : 'Минимум 6 символов'),
          if (!_isLogin) ...[
            const SizedBox(height: 16),
            TextFormField(controller: _confirmPasswordController, decoration: InputDecoration(labelText: 'Подтвердите пароль', prefixIcon: const Icon(Icons.lock), suffixIcon: IconButton(icon: Icon(_obscureConfirmPassword ? Icons.visibility : Icons.visibility_off), onPressed: () => setState(() => _obscureConfirmPassword = !_obscureConfirmPassword)), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), filled: true, fillColor: Colors.grey.shade50), obscureText: _obscureConfirmPassword, validator: (v) => v != _passwordController.text ? 'Пароли не совпадают' : null),
            const SizedBox(height: 16),
            TextFormField(controller: _adminPhraseController, decoration: InputDecoration(labelText: 'Код администратора (если есть)', prefixIcon: const Icon(Icons.admin_panel_settings), border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)), filled: true, fillColor: Colors.grey.shade50)),
          ],
          if (_isLogin) ...[
            Row(children: [Checkbox(value: authProvider.rememberMe, onChanged: (v) => authProvider.setRememberMe(v!)), const Text('Запомнить меня')]),
            const SizedBox(height: 8),
            Align(alignment: Alignment.centerRight, child: TextButton(onPressed: _showForgotPasswordDialog, child: const Text('Забыли пароль?'))),
          ],
        ])),
        const SizedBox(height: 24),
        if (_isLoading) const Center(child: CircularProgressIndicator()) else ElevatedButton(onPressed: _submit, style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: Text(_isLogin ? 'Войти' : 'Зарегистрироваться', style: const TextStyle(fontSize: 16))),
        const SizedBox(height: 16),
        TextButton(onPressed: () { setState(() { _isLogin = !_isLogin; _profileImage = null; _adminPhraseController.clear(); }); }, child: Text(_isLogin ? 'Нет аккаунта? Зарегистрироваться' : 'Уже есть аккаунт? Войти')),
        const SizedBox(height: 20),
      ]))),
    );
  }
...
class _MainScreenState extends State<MainScreen> {
  ...
  @override Widget build(BuildContext context) {
    final notifProvider = Provider.of<NotificationProvider>(context);
    return Scaffold(
      appBar: AppBar(title: Text(_titles[_selectedIndex]), actions: _selectedIndex == 0 || _selectedIndex == 1 ? [
        IconButton(icon: const Icon(Icons.search), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const SearchScreen()))),
        IconButton(icon: const Icon(Icons.share), onPressed: () => _generateInventoryReport(context)),
      ] : null),
      body: _screens[_selectedIndex],
      bottomNavigationBar: BottomNavigationBar(
        currentIndex: _selectedIndex, onTap: (i) => setState(() => _selectedIndex = i),
        type: BottomNavigationBarType.fixed, selectedItemColor: Theme.of(context).colorScheme.primary, unselectedItemColor: Colors.grey[600],
        items: const [
  ...
  Future<void> _generateInventoryReport(BuildContext context) async {
    ...
    ReportService.showReportTypeDialog(context, Tool(id: 'inventory', title: 'Инвентаризация', description: '', brand: '', uniqueId: '', currentLocation: '', currentLocationName: '', userId: FirebaseAuth.instance.currentUser?.uid ?? 'local'), (type) async {
...
class _ToolsListScreenState extends State<ToolsListScreen> {
  ...
  @override Widget build(BuildContext context) {
    ...
    final auth = Provider.of<AuthProvider>(context);
    return Scaffold(
      appBar: AppBar(title: Text('Все инструменты (${tp.totalTools})'), actions: [
        IconButton(icon: const Icon(Icons.filter_list), onPressed: () => _showFilterDialog(context)),
        IconButton(icon: const Icon(Icons.refresh), onPressed: () => tp.loadTools(forceRefresh: true)),
      ]),
      body: tp.isLoading && tp.tools.isEmpty ? _buildLoadingScreen() : Column(children: [
        Padding(padding: const EdgeInsets.all(12.0), child: TextField(controller: _searchController, decoration: InputDecoration(hintText: 'Поиск инструментов...', prefixIcon: const Icon(Icons.search), border: OutlineInputBorder(borderRadius: BorderRadius.circular(10)), filled: true, fillColor: Colors.grey.shade50), onChanged: tp.setSearchQuery)),
        if (tp.filterLocation != 'all' || tp.filterBrand != 'all' || tp.filterFavorites) Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.centerLeft, end: Alignment.centerRight, colors: [Theme.of(context).colorScheme.primary.withOpacity(0.1), Theme.of(context).colorScheme.secondary.withOpacity(0.1)])), child: Row(children: [
          Icon(Icons.filter_alt, size: 16, color: Theme.of(context).colorScheme.primary), const SizedBox(width: 8),
          Expanded(child: Text(_getActiveFiltersText(tp), style: TextStyle(fontSize: 12, color: Theme.of(context).colorScheme.primary))),
          TextButton(onPressed: tp.clearAllFilters, child: const Text('Очистить', style: TextStyle(fontSize: 12))),
        ])),
        Expanded(child: tp.tools.isEmpty ? _buildEmptyToolsScreen(auth.isAdmin) : ListView.builder(itemCount: tp.tools.length, itemBuilder: (context, i) {
          final tool = tp.tools[i];
          return SelectionToolCard(tool: tool, selectionMode: tp.selectionMode, onTap: () => Navigator.push(context, MaterialPageRoute(builder: (context) => EnhancedToolDetailsScreen(tool: tool))));
        })),
      ]),
      floatingActionButton: tp.selectionMode
        ? FloatingActionButton.extended(onPressed: tp.hasSelectedTools ? () => _showSelectionActions(context) : null, icon: const Icon(Icons.more_vert), label: Text('${tp.selectedTools.length}'), backgroundColor: Theme.of(context).colorScheme.primary)
        : (auth.isAdmin ? FloatingActionButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const AddEditToolScreen())), backgroundColor: Theme.of(context).colorScheme.primary, child: const Icon(Icons.add)) : null),
      floatingActionButtonLocation: FloatingActionButtonLocation.centerFloat,
    );
  }
  Widget _buildLoadingScreen() => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Theme.of(context).colorScheme.primary)), const SizedBox(height: 20), Text('Загрузка инструментов...', style: TextStyle(fontSize: 16, color: Colors.grey[600]))]));
  Widget _buildEmptyToolsScreen(bool isAdmin) => Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.build, size: 80, color: Colors.grey.shade300), const SizedBox(height: 20), const Text('Нет инструментов', style: TextStyle(fontSize: 18, color: Colors.grey)), const SizedBox(height: 10), if (isAdmin) ElevatedButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const AddEditToolScreen())), child: const Text('Добавить инструмент'))]));
  String _getActiveFiltersText(ToolsProvider p) {
    List<String> f = [];
    if (p.filterLocation != 'all') f.add(p.filterLocation == 'garage' ? 'В гараже' : 'На объекте');
    if (p.filterBrand != 'all') f.add('Бренд: ${p.filterBrand}');
    if (p.filterFavorites) f.add('Избранные');
    return f.join(', ');
  }
      ...
      return StatefulBuilder(builder: (context, setState) {
        return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(borderRadius: const BorderRadius.vertical(top: Radius.circular(20)), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.white, Colors.grey.shade50])), child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            ExpansionTile(title: const Text('Местоположение'), children: [
              ...
              RadioListTile<String>(title: const Text('Гараж'), value: 'garage', groupValue: tp.filterLocation, onChanged: (v) { setState(() {}); tp.setFilterLocation(v!); }),
              ...op.objects.map((obj) => RadioListTile<String>(title: Text(obj.name), value: obj.id, groupValue: tp.filterLocation, onChanged: (v) { setState(() {}); tp.setFilterLocation(v!); })),
            ]),
            ExpansionTile(title: const Text('Бренд'), children: tp.uniqueBrands.map((b) => RadioListTile<String>(title: Text(b == 'all' ? 'Все' : b), value: b, groupValue: tp.filterBrand, onChanged: (v) { setState(() {}); tp.setFilterBrand(v!); })).toList()),
            SwitchListTile(title: const Text('Только избранные'), value: tp.filterFavorites, onChanged: tp.setFilterFavorites),
            const SizedBox(height: 20), Row(children: [Expanded(child: TextButton(onPressed: tp.clearAllFilters, child: const Text('Сбросить все'))), const SizedBox(width: 10), Expanded(child: ElevatedButton(onPressed: () => Navigator.pop(context), child: const Text('Применить')))]),
          ]));
      });
    });
  }
  void _showSelectionActions(BuildContext context) {
    ...
    showModalBottomSheet(context: context, builder: (context) {
      return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(borderRadius: const BorderRadius.vertical(top: Radius.circular(20)), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Colors.white, Colors.grey.shade50])), child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 20), TextButton(onPressed: () => Navigator.pop(context), child: const Text('Отмена')),
        ]));
    });
  }
  void _showMultiDeleteDialog(BuildContext context) {
...
class MyApp extends StatelessWidget {
    ...
    return FutureBuilder<SharedPreferences>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return MaterialApp(home: Scaffold(body: Center(child: CircularProgressIndicator())), debugShowCheckedModeBanner: false);
        }
        ...
        return MultiProvider(
          ...
          child: Consumer<AuthProvider>(builder: (context, authProvider, _) {
            ...
            return MaterialApp(
              ...
              themeMode: themeMode == 'dark' ? ThemeMode.dark : themeMode == 'system' ? ThemeMode.system : ThemeMode.light,
              ...
              home: StreamBuilder<User?>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (user == null) {
                    ...
                    if (!seenWelcome) {
                      return WelcomeScreen(onContinue: () async { await prefs.setBool('seen_welcome', true); });
                    }
                    final seenOnboarding = prefs.getBool('seen_onboarding') ?? false;
                    if (!seenOnboarding) {
                      return OnboardingScreen(onComplete: () async { await prefs.setBool('seen_onboarding', true); });
                    }
  ...
  ThemeData _buildLightTheme() => ThemeData(
    brightness: Brightness.light, primaryColor: const Color(0xFF24292e),
    colorScheme: const ColorScheme.light(primary: Color(0xFF24292e), secondary: Color(0xFF0366d6), surface: Color(0xFFffffff), background: Color(0xFFf6f8fa), error: Color(0xFFd73a49)),
    appBarTheme: const AppBarTheme(elevation: 0, backgroundColor: Color(0xFF24292e), iconTheme: IconThemeData(color: Colors.white), titleTextStyle: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w600, letterSpacing: -0.5)),
    bottomNavigationBarTheme: const BottomNavigationBarThemeData(backgroundColor: Colors.white, selectedItemColor: Color(0xFF0366d6), unselectedItemColor: Color(0xFF586069)),
    floatingActionButtonTheme: const FloatingActionButtonThemeData(backgroundColor: Color(0xFF2ea44f), foregroundColor: Colors.white),
    elevatedButtonTheme: ElevatedButtonThemeData(style: ElevatedButton.styleFrom(foregroundColor: Colors.white, backgroundColor: const Color(0xFF2ea44f), textStyle: const TextStyle(fontWeight: FontWeight.w500), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12))),
    inputDecorationTheme: InputDecorationTheme(filled: true, fillColor: Colors.white, border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: const BorderSide(color: Color(0xFFe1e4e8))), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: const BorderSide(color: Color(0xFFe1e4e8))), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: const BorderSide(color: Color(0xFF0366d6), width: 2)), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 14)),
    cardTheme: CardThemeData(elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6), side: const BorderSide(color: Color(0xFFe1e4e8)))),
    dividerTheme: const DividerThemeData(color: Color(0xFFe1e4e8), thickness: 1, space: 0),
    textTheme: const TextTheme(
      displayLarge: TextStyle(color: Color(0xFF24292e), fontWeight: FontWeight.w600, fontSize: 32),
      displayMedium: TextStyle(color: Color(0xFF24292e), fontWeight: FontWeight.w600, fontSize: 24),
      displaySmall: TextStyle(color: Color(0xFF24292e), fontWeight: FontWeight.w600, fontSize: 20),
      titleLarge: TextStyle(color: Color(0xFF24292e), fontWeight: FontWeight.w600, fontSize: 18),
      titleMedium: TextStyle(color: Color(0xFF24292e), fontWeight: FontWeight.w500, fontSize: 16),
      titleSmall: TextStyle(color: Color(0xFF586069), fontWeight: FontWeight.w500, fontSize: 14),
    ...
    chipTheme: ChipThemeData(backgroundColor: const Color(0xFFf6f8fa), labelStyle: const TextStyle(color: Color(0xFF24292e), fontSize: 12), padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6), side: const BorderSide(color: Color(0xFFe1e4e8)))),
  );
  ThemeData _buildDarkTheme() => ThemeData(
    brightness: Brightness.dark, primaryColor: const Color(0xFF0d1117),
    colorScheme: const ColorScheme.dark(primary: Color(0xFF58a6ff), secondary: Color(0xFF1f6feb), surface: Color(0xFF161b22), background: Color(0xFF0d1117), error: Color(0xFFf85149)),
    appBarTheme: const AppBarTheme(elevation: 0, backgroundColor: Color(0xFF161b22), iconTheme: IconThemeData(color: Color(0xFFc9d1d9)), titleTextStyle: TextStyle(color: Color(0xFFc9d1d9), fontSize: 20, fontWeight: FontWeight.w600, letterSpacing: -0.5)),
    bottomNavigationBarTheme: const BottomNavigationBarThemeData(backgroundColor: Color(0xFF161b22), selectedItemColor: Color(0xFF58a6ff), unselectedItemColor: Color(0xFF8b949e)),
    floatingActionButtonTheme: const FloatingActionButtonThemeData(backgroundColor: Color(0xFF238636), foregroundColor: Colors.white),
    elevatedButtonTheme: ElevatedButtonThemeData(style: ElevatedButton.styleFrom(foregroundColor: Colors.white, backgroundColor: const Color(0xFF238636), textStyle: const TextStyle(fontWeight: FontWeight.w500), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)), padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12))),
    inputDecorationTheme: InputDecorationTheme(filled: true, fillColor: const Color(0xFF0d1117), border: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: const BorderSide(color: Color(0xFF30363d))), enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: const BorderSide(color: Color(0xFF30363d))), focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(6), borderSide: const BorderSide(color: Color(0xFF58a6ff), width: 2)), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 14)),
    cardTheme: CardThemeData(elevation: 0, color: const Color(0xFF161b22), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6), side: const BorderSide(color: Color(0xFF30363d)))),
    dividerTheme: const DividerThemeData(color: Color(0xFF21262d), thickness: 1, space: 0),
    textTheme: const TextTheme(
      displayLarge: TextStyle(color: Color(0xFFc9d1d9), fontWeight: FontWeight.w600, fontSize: 32),
      displayMedium: TextStyle(color: Color(0xFFc9d1d9), fontWeight: FontWeight.w600, fontSize: 24),
      displaySmall: TextStyle(color: Color(0xFFc9d1d9), fontWeight: FontWeight.w600, fontSize: 20),
      titleLarge: TextStyle(color: Color(0xFFc9d1d9), fontWeight: FontWeight.w600, fontSize: 18),
      titleMedium: TextStyle(color: Color(0xFFc9d1d9), fontWeight: FontWeight.w500, fontSize: 16),
      titleSmall: TextStyle(color: Color(0xFF8b949e), fontWeight: FontWeight.w500, fontSize: 14),
    ...
    chipTheme: ChipThemeData(backgroundColor: const Color(0xFF21262d), labelStyle: const TextStyle(color: Color(0xFFc9d1d9), fontSize: 12), padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6), side: const BorderSide(color: Color(0xFF30363d)))),
  );
}